 ...
 * Breadcrumbs
 */
if ( ! function_exists( 'sodalicious_get_breadcrumbs' ) ) {
	function sodalicious_get_breadcrumbs() {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'sodalicious' ); // text for the 'Home' link
		$text['category'] = esc_html__( 'Archive by Category "%s"', 'sodalicious' ); // text for a category page
		$text['search']   = esc_html__( 'Search Results for "%s"', 'sodalicious' ); // text for a search results page
		$text['tag']      = esc_html__( 'Posts Tagged "%s"', 'sodalicious' ); // text for a tag page
		$text['author']   = esc_html__( 'Articles Posted by %s', 'sodalicious' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'sodalicious' ); // text for the 404 page

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'sodalicious' ); // text for the 'Home' link
		if( get_option( 'page_for_posts' ) !=='0' ){
		$text['category'] = '<a href="'.get_permalink( get_option( 'page_for_posts' ) ) . '">' . esc_html__( 'Blog', 'sodalicious' ) . '</a> <span class="sep">-</span> ' . esc_html__( '%s', 'sodalicious' ); // text for a category page
		}
		$text['search']   = esc_html__( 'Search Results for "%s"', 'sodalicious' ); // text for a search results page
		$text['tag']      = esc_html__( '%s', 'sodalicious' ); // text for a tag page
		$text['author']   = esc_html__( 'Articles Posted by %s', 'sodalicious' ); // text for an author page
		$text['404']      = esc_html__( '404', 'sodalicious' ); // text for the 404 page
		$text['page']     = esc_html__( 'Page %s', 'sodalicious' ); // text 'Page N'
		$text['cpage']    = esc_html__( 'Comment Page %s', 'sodalicious' ); // text 'Comment Page N'
		...
		if ( is_home() || is_front_page() ) {
			...
			if ( is_category() ) {
				...
				if ( get_post_type() != 'post' ) {
					...
					if ( get_option( 'page_for_posts' ) !=='0' ) {
						$output .= '<a href="'.get_permalink( get_option( 'page_for_posts' ) ) . '">' . esc_html__( 'Blog', 'sodalicious' ) . '</a> <span class="sep">-</span> '. $cats;
					} else {
						$output .= $cats;
					}
